...
#### Scenario: Check-in and check-out dates are displayed in hotel timezone
- **WHEN** the modal displays check-in and check-out dates
- **THEN** the dates are formatted using `formatInTimezone` from `common/utils/clientTimezone.ts` with the hotel timezone from `useHotelTimezone()`, not the browser's local timezone
