...
export async function GET(req: NextRequest) {
  try {
    ...
    const items = await prisma.expenseCategory.findMany({ where, orderBy: { name: "asc" } });
    return ok(items);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const existing = await prisma.expenseCategory.findUnique({ where: { name: body.name.trim() } });
    if (existing) {
      if (!existing.isActive) {
        return conflict("An inactive category with this name already exists", "EXPENSE_CATEGORY_INACTIVE_EXISTS", { id: existing.id });
      }
