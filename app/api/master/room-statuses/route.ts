...
export async function GET(req: NextRequest) {
  try {
    ...
    const [items, total] = await Promise.all([
      prisma.roomStatus.findMany({ where, skip: (page - 1) * limit, take: limit, orderBy: { name: "asc" } }),
      prisma.roomStatus.count({ where }),
    ]);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const item = await prisma.roomStatus.create({
      data: { code: body.code, name: body.name, color: body.color ?? "#6b7280", isSellable: body.isSellable ?? false },
    });
    return created(item);
  } catch (e) { return serverError(e); }
}
